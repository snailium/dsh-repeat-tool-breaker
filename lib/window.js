...
export function createTracker(cfg) {
  ...
  function slot(agent) {
    ...
    if (state === undefined) {
      state = { calls: [], lastResult: '', localExempt: false, localRefused: false }
  ...
  /**
   * The three pieces of local-address state, all scoped to the CURRENT turn
   * (`reset` clears them with everything else):
   *
   *   - `exempt`   — set when the operator approved an `ask`, or when the policy
   *                  is `allow`. Local target fingerprints stop blocking.
   *   - `refused`  — set when an `ask` was rejected. Further local blocks are
   *                  denied without asking again, so a refusal cannot turn into a
   *                  prompt loop.
   *
   * `stalled` is the pending-ask bookkeeping: a call whose `ask` was approved
   * never reaches the guard, so "the guard saw this execution" is exactly the
   * approval signal (see index.js).
   */
  function isLocalExempt(agent) {
    return peek(agent)?.localExempt === true
  }

  function exemptLocal(agent) {
    slot(agent).localExempt = true
  }

  function isLocalRefused(agent) {
    return peek(agent)?.localRefused === true
  }

  function refuseLocal(agent) {
    slot(agent).localRefused = true
  }

  return { wouldExceed, commit, reset, slot, isLocalExempt, exemptLocal, isLocalRefused, refuseLocal }
}
