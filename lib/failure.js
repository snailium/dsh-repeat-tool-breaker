...
function classifyFromText(result, command) {
  ...
  // `curl -w "%{http_code}"` (and `%{response_code}`) makes the command itself the
  // evidence: the status is in the output BY CONSTRUCTION, so a leading 4xx/5xx is the
  // status and not a coincidence. This is the shape a model produces when it checks an
  // endpoint by hand, and it is invisible otherwise: curl exits 0 for a 404 unless it
  // was given `--fail`, the text is a bare number, and none of the markers below match.
  // Anchored to the start, which is where `-w` puts the value.
  if (typeof command === 'string' && /%\{(?:http_code|response_code)\}/.test(command)) {
    const status = /^\s*([45]\d\d)(?:\s|$)/.exec(text)
    if (status !== null) return { reason: 'http', detail: status[1] }
  }
