 ...
 * @param options - `{ shell, command }`: whether the call was a SHELL tool, and the
 *   command it ran. The command matters because it is EVIDENCE about its own output:
 *   a `curl -w "%{http_code}"` prints the status by construction, so a leading 4xx/5xx
 *   in the output is the status rather than a coincidence. The text fallback
...
export function classifyFailure(result, options = {}) {
  ...
  // "no answer", not "guess".
  return options.shell === true ? classifyFromText(result, options.command) : null
}
...
export function describeFailure(failure) {
  ...
  switch (failure.reason) {
    ...
    case 'curl':
      return detail ? `curl failed (code ${detail})` : 'curl failed'
    default:
...
/**
 * Pull the `-w` / `--write-out` format out of a shell command, or `null`.
 *
 * Only the LAST one is used, matching curl: a later `-w` replaces an earlier one.
 * Quoted forms are preferred, but a bare value works too, and `--write-out=X` is
 * accepted. A bundled short flag (`-sw`) is deliberately NOT matched — it falls back
 * to the licensed scan below, which is the safer failure mode.
 *
 * @param command - the shell command.
 * @returns the format string, or `null` when there is no write-out.
 */
function writeOutFormat(command) {
  const pattern = /(?:^|[\s;&|(])(?:--write-out(?:=|\s+)|-w\s*)(?:"([^"]*)"|'([^']*)'|(\S+))/g
  let last = null
  let match
...
/**
 * The status code curl's write-out would have printed, read from the tail of the
 * output. `-w` is appended after the body, so the format-derived pattern is anchored
 * at the end; every placeholder other than the status becomes a wildcard.
 *
 * @param text - the settled output.
 * @param format - the `-w` format string.
 * @returns the status code, or `null` when the pattern does not match the tail.
 */
function statusFromWriteOut(text, format) {
  ...
  let source = ''
  let captures = 0
  for (const part of expanded.split(/(%\{[A-Za-z_]+\})/)) {
    ...
    if (name === 'http_code' || name === 'response_code') {
      source += '(\\d{3})'
      captures += 1
    } else {
      source += '[\\s\\S]*?'
  ...
  const match = new RegExp(`${source}\\s*$`).exec(text)
  if (match === null) return null
  const found = match.slice(1).filter((value) => value !== undefined)
  return found.length === 0 ? null : Number(found[found.length - 1])
}

/**
 * The last standalone 4xx/5xx in the output, or `null`.
 *
 * Used only when the command demonstrably asked for a status and the format could not
 * be located. Last rather than first because `-w` appends: in `…body…200\ndone` the
 * status is the 200, and the body's own `404` is not.
 *
 * @param text - the settled output.
 * @returns the status code, or `null`.
 */
function lastStatusToken(text) {
  const pattern = /(?:^|[^\d])([45]\d\d)(?!\d)/g
  let last = null
  let match
...
function classifyFromText(result, command) {
  ...
  // An HTTP status LINE (`curl -I`, or a HEAD request): unambiguous, and the only marker
  // here that needs no help from the command.
  const statusLine = /\bHTTP\/\d(?:\.\d)?\s+([45]\d\d)\b/.exec(text)
  if (statusLine !== null) return { reason: 'http', detail: statusLine[1] }

  // `curl -w "%{http_code}"` and friends. The command is the evidence: it asked for the
  // status, so the status is in the output BY CONSTRUCTION — and curl exits 0 for a 404
  // unless it was given `--fail`, so nothing else would notice.
  //
  // Where it appears is NOT fixed: the format string is arbitrary, and an earlier
  // revision of this function only looked at the START of the output, which caught the
  // one shape that happened to be reported and missed every other. `-w` output is
  // APPENDED after the body, so the format string itself says where the status sits:
  // parse it, and match the tail of the output.
  if (typeof command === 'string' && /%\{(?:http_code|response_code)\}/.test(command)) {
    const format = writeOutFormat(command)
    const fromFormat = format === null ? null : statusFromWriteOut(text, format)
    // A parsed format is definitive, including when it says 200 — do not then scan the
    // body for a number that merely looks like a status. The scan is the fallback for a
    // command that appended something AFTER curl, so the format no longer ends the
    // output.
    const status = fromFormat ?? lastStatusToken(text)
    if (status !== null && status >= 400) return { reason: 'http', detail: String(status) }
  }
