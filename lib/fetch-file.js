...
/**
 * Live state the guard reads. `registered` is the one that matters: the guard may
 * only name `web_fetch_file` when it is actually callable in this profile.
 */
export const fetchFileToolState = { registered: false }
 ...
 * @returns nothing; inspect {@link fetchFileToolState}.
 */
export function registerFetchFileTool(ctx, settings) {
  ...
  ctx.inject(['web'], (webCtx) => {
    ...
    fetchFileToolState.registered = true
  })
}
