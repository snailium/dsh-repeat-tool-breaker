...
export const DEFAULTS = deepFreeze({
  ...
  blockShellHttp: true,
  /**
   * Verbs the HTTP block leaves alone. Two different reasons put a verb here, and the
   * distinction matters when deciding whether to add another.
   *
   * **Incidental network use, evidence-led** — `git`, `docker`. A scan of the recorded
   * corpus (`tools/shell-http-allowlist-scan.mjs`) found these are the only non-curl
   * verbs that ever carry a remote URL, and refusing them would remove a capability
   * rather than redirect one. The URL-less forms are not blocked anyway (`npm install
   * foo` carries no address), so `wget`, `npm`, `pip`, `uv`, `apt`, `go`, `cargo` and
   * `gh` are NOT exempted on speculation. `wget` is deliberately absent even though it
   * appears: it has the same fetch-to-file equivalent `curl` does.
   *
   * **URL-shaped PATTERN, not a destination** — `grep`, `rg`. These take a regex as
   * their primary argument, so a `grep` command containing a URL is almost always
   * searching text FOR that address, and no amount of it can fetch anything. Refusing
   * it does not redirect a fetch; it blocks a read, and the refusal message then states
   * that the call "fetches over HTTP", which is simply false. A guard that lies about
   * why it fired is worse than one that is narrow.
   *
   * The line is PATTERN-POSITION tools, not "everything without a network stack": `echo`,
   * `head`, `cat`, `sed` and `awk` are excluded, because the corpus contains commands
   * where one of those merely PRINTS a URL that a later segment then downloads — there
   * the exemption would swallow a real fetch. `grep` was checked the same way with
   * `tools/allowlist-candidate-scan.mjs`: exempting it flips no refusal in the corpus,
   * because whenever grep carries a remote URL, another verb in the same command is
   * refused too.
   *
   * KNOWN RESIDUAL: `grep -oE '<url>' f | xargs curl` becomes allowed, since the only
   * segment carrying an address is the exempt grep. This is the same class as the hole
   * already asserted in T47b, and it is not new: `U=$(grep -oE '<url>' f); curl "$U"` is
   * allowed TODAY, because the assignment makes the first token `U=$(grep` — not a
   * plausible command name, so the segment is skipped entirely. The block is a steering
   * mechanism, not a containment boundary, and the committed corpus shows it guiding
   * behaviour rather than being evaded.
   *
   * Set it to `[]` to refuse every verb, including these.
   */
  shellHttpAllow: ['git', 'docker', 'grep', 'rg'],
  /**
