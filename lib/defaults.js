...
export const DEFAULTS = deepFreeze({
   ...
   *
   *   - `deny`  (default) — local calls are counted and blocked like any other,
   *              and the denial names this knob. The right choice for a profile
   *              with nobody watching (headless), where an open question would
   *              simply stall the turn.
   *   - `ask`   — the first local call that would be blocked instead asks the
   *              operator, once per turn; approving exempts local traffic (the
   *              target-scoped fingerprints) for the REST OF THAT TURN, and a
   *              refusal stops asking and behaves like `deny` until the next
   *              human message. Needs an approval service AND an answerer, so it
   *              is only sane in a profile with a UI.
