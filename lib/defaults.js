...
export const DEFAULTS = deepFreeze({
   ...
   * Every cap must be a finite number >= 2, so the first call always survives.
   *
   * The precise caps (`exact`, `cmd`, `net`, `sink`, `readpath`, `writepath`)
   * are what actually catch a loop. The volume caps (`site`, `family:*`,
   * `verb:*`) are budgets, not repetition detectors, and a measured run showed
   * the cost of setting them tight: a task that fetched four DIFFERENT URLs with
   * `curl` tripped `family:http-fetch: 4` on the fourth. They are kept as a
