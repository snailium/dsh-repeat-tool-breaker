...
export const DEFAULTS = deepFreeze({
   ...
   * local operation and there is no fetch-to-file replacement for them: `git clone`,
   * `npm install git+https://…`, `docker pull`, `pip install`, `apt-get`.
   *
   * This is a deliberate exception, not leniency. The block exists so that a fetch
   * whose OUTCOME matters becomes a structured fact; a package manager's fetch is not
   * something the model then reasons about page by page, and blocking it would remove
   * a capability rather than redirect one. Set it to `[]` to block those too.
   */
  shellHttpAllow: ['git', 'npm', 'pnpm', 'yarn', 'docker', 'podman', 'pip', 'pip3', 'uv', 'apt', 'apt-get', 'cargo', 'go', 'gh'],
  /**
