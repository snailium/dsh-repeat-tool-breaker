 ...
 * allowed.
 *
 * ## The three escalating stages (0.4.0)
 *
 * `limits` is the only threshold for the hard step; the two advisory stages sit
 * below it and never block:
 *
 *   - `warnAt`      (3) — a light warning: you are repeating, consider another route
 *   - `summarizeAt` (6) — a demand: summarise progress, list untried alternatives
 *   - `limits[k]`   (9) — the gate: ask the operator (`onLimit: ask`) or deny
 *
...
export const DEFAULTS = deepFreeze({
   ...
   * catch a loop. 9 rather than 5 because two advisory stages now sit underneath:
   * 3 warns, 6 demands a summary, 9 gates. (2 was too low — it blocked the
   * ordinary retry after a failure unrelated to looping; 3 fixed that; 5 still
   * tripped dense legitimate work, and 9 leaves room for the escalations.)
   *
