...
export const DEFAULTS = deepFreeze({
  ...
  blockShellHttp: true,
  /**
   * The commands the HTTP block REFUSES. This is the blacklist, and it is the whole policy:
   * the tool traverses every place the command line can run something and refuses when any
   * candidate's verb is on this list.
   *
   * A command is refused because it RUNS something that fetches, never because it CONTAINS
   * an address — an address lives in grep patterns, heredocs and commit messages, and
   * refusing those blocked ordinary work while asserting, falsely, that they fetched.
   *
   * The first four are the raw HTTP clients; the rest are the file/streaming downloaders,
   * which are the same family and have the same fetch-to-file replacement. `wget` is on the
   * list rather than off: it does have `web_fetch_file` as an equivalent.
   *
   * Interpreters are NOT here and cannot be: `python3` would refuse every local script. They
   * are covered by the request-API arm instead (`urllib`, `requests.`, `fetch(`, …), which
   * is why a `python3 -c` one-liner is still stopped.
   *
   * Coverage is extensible by design. Add a command here from Settings → Plugins; add a
   * shell to open (`sh`, `ssh`, `xargs`, `find -exec`, …) in `lib/normalize.js`. Set the list
   * to `[]` to switch the block off without touching `blockShellHttp`.
   *
   * `socat` is deliberately ABSENT even though it is the only verb the coverage scan misses
   * (20 corpus calls, `tools/block-coverage.mjs`). Not one of those calls fetches anything:
   * they are `socat -V`, `socat -h`, and a local TLS relay. Adding it would refuse a version
   * query, which is exactly the false-positive class this list exists to avoid. Covering
   * socat properly means judging whether it OPENS A CONNECTION, not whether its name is
   * listed — a different kind of rule, for a later pass.
   */
  shellHttpBlock: ['curl', 'wget', 'http', 'httpie', 'aria2c', 'rclone', 'yt-dlp', 'youtube-dl'],
...
const REMOVED_KEYS = Object.freeze({
  shellHttpAllow:
    'it was REPLACED in 0.7.0 by `shellHttpBlock`, the list of commands to REFUSE — the ' +
    'block is a blacklist now, so an exemption list has no meaning',
...
export function mergeDefaults(config = {}) {
  ...
  for (const key of ['exclude', 'include', 'shellHttpBlock']) {
