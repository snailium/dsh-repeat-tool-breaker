...
export const DEFAULTS = deepFreeze({
   ...
   * loop. The volume caps (`site`, `family:*`, `verb:*`) are budgets, not
   * repetition detectors, and a measured run showed the cost of setting them
   * tight: a task that fetched four DIFFERENT URLs with `curl` tripped
   * `family:http-fetch: 4` on the fourth. They are kept as a runaway-crawl
   * backstop, one above the largest ordinary batch.
   *
