 ...
 *
 * @param options - `{ name, toolAvailable, localAllowed }`.
 * @returns the model-facing denial text.
 */
export function shellHttpBlockedMessage({ name, toolAvailable, localAllowed }) {
  const lines = [
  ...
  if (toolAvailable) {
    ...
    lines.push(
      '',
      'No fetch-to-file tool is registered in this profile, so there is no shell-free way',
      'to make this request; the call is refused rather than failed silently.',
    )
  }
  if (localAllowed) {
    lines.push(
