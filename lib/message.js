...
export function denyMessage({ name, hits, lastResult, cfg, local }) {
  ...
  if (local === 'refused') {
    lines.push(
      '',
      'Every hit above points at a LOCAL address (localhost / private network). The operator',
      'was asked to exempt local traffic for this turn and declined, so local calls stay',
      'blocked until the next human message.',
    )
